-- Top Dragon TMS — Etap 3L.36
-- Podsumowania tygodniowe na tablicy + wyrównania per kierowca + wielostopniowa akceptacja przelewów.
-- Zasady przelewu wyniku: odbiorca + kierownik oddziału nadawcy + kierownik oddziału odbiorcy.
-- Usunąć (soft-delete) przelew może wyłącznie spedytor-odbiorca; zachowujemy godzinę usunięcia.
...
alter table public.tms_dispatcher_week_transfers
  ...
  add column if not exists recipient_approved_by uuid null references public.profiles(id) on delete set null,
  add column if not exists from_manager_approved_at timestamptz null,
  add column if not exists from_manager_approved_by uuid null references public.profiles(id) on delete set null,
  add column if not exists to_manager_approved_at timestamptz null,
  add column if not exists to_manager_approved_by uuid null references public.profiles(id) on delete set null,
  add column if not exists deleted_at timestamptz null,
  add column if not exists deleted_by uuid null references public.profiles(id) on delete set null,
  add column if not exists rejected_at timestamptz null,
  add column if not exists rejected_by uuid null references public.profiles(id) on delete set null;

alter table public.tms_dispatcher_week_transfers
  drop constraint if exists tms_dispatcher_week_transfers_status_check;

-- Historyczne anulowania konwertujemy przed założeniem nowego ograniczenia statusów.
...
-- Zachowujemy historyczne zaakceptowane przelewy jako zaakceptowane po migracji.
update public.tms_dispatcher_week_transfers
